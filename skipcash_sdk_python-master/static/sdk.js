var skipcash = {
  sdk: {
    ...
    create: function (options) {
      ...
      options.btnHoverStyle = {
        ...defaultBtnHoverStyle,
        ...options.btnHoverStyle,
      };
      options.logoImgStyle = { ...defaultImgStyle, ...options.logoImgStyle };
      options.logoImgHoverStyle = {
        ...defaultImgHoverStyle,
        ...options.logoImgHoverStyle,
      };
      if (options.direct) {
        options.btnInnerHtml = "Direct pay with";
      }
      ...
      if (options.container == null) {
        if (options.onError != null)
          options.onError("Container is a required parameter");
        return;
      }
      if (options.clientId == null) {
        if (options.onError != null)
          options.onError("clientId is a required parameter");
        return;
      }
      if (options.onCreatePayment == null) {
        if (options.onError != null)
          options.onError("onCreatePayment is a required parameter");
        return;
      }
      if (options.onSuccess == null) {
        if (options.onError != null)
          options.onError("onSuccess is a required parameter");
        return;
      }
      if (options.environment == "sandbox") {
        options.baseUrl = options.sandBoxUrl;
      } else if (options.environment == "production") {
        options.baseUrl = options.productionUrl;
      } else {
        if (options.onError != null)
          options.onError("Invalid environment value");
        return;
      }
      ...
      if (container != null) {
        ...
        btn.innerHTML = options.btnInnerHtml;
        ...
        styles.innerHTML =
          "." + btnClass + " { " + getStyleString(options.btnStyle) + " } ";
        styles.innerHTML +=
          "." +
          btnClass +
          ":hover { " +
          getStyleString(options.btnHoverStyle) +
          " } ";
        styles.innerHTML +=
          "." +
          logoClass +
          " { " +
          getStyleString(options.logoImgStyle) +
          " } ";
        styles.innerHTML +=
          "." +
          btnClass +
          ...
          logoClass +
          " { " +
          getStyleString(options.logoImgHoverStyle) +
          " } ";
          ...
          if (options.direct) {
            options
              ...
              .then((value) => {
                ...
                window.location.href = options.baseUrl + options.payUrl + value;
              })
              .catch((err) => {
                if (options.onError != null)
                  options.onError("Error when creating payment: " + err);
              });
          } else {
            var newWindow = options.openPopup("about:blank");
            if (!newWindow) {
              alert(
                "Failed to open a popup window for SkipCash payment! Please check your browser settings to allow popups."
              );
              return;
            }
            options
              ...
              .then((value) => {
                ...
                newWindow.location.href =
                  options.baseUrl + options.payUrl + value;
        ...
        options.onWindowClose = function (newWindow) {
          if (newWindow.closed) {
            fetch(
              options.baseUrl + "api/v1/payments/" + options.transactionId,
              {
                method: "GET",
                //mode: 'no-cors',
                headers: new Headers({
                  Authorization: options.clientId,
                }),
              }
            )
                ...
                if (result.returnCode == 200 && result.resultObj != null) {
                  ...
                  if (options.onError != null) {
                    if (result.errorMessage)
                      options.onError(
                        "Failed to retrieve information about the payment: " +
                          result.errorMessage
                      );
                    else
                      options.onError(
    ...
    payWithApplePay: async function (options, onSuccess, onError) {
      ...
      //       options.onValidateSession(
      //         url,
      //         function (merchantSession) {
      //         console.log("merchantSession",merchantSession);
      //       // console.log("completeMerchantValidation",session.completeMerchantValidation(merchantSession));

      //           session.completeMerchantValidation(merchantSession);
      //         },
      //         function (e) {
      //           if (onError != null) onError("Validation failed: " + e);
      //         }
      //       );
      //     },
      //     function (e) {
      ...
      session.onpaymentauthorized = (e) => {
        ...
        callApplePay(
          ...
          (isSuccess) => {
            console.log("onpaymentauthorized isSuccess time",applePaymentToken);
            console.log("ApplePaySession.STATUS_SUCCESS",ApplePaySession.STATUS_SUCCESS);
            console.log("ApplePaySession.STATUS_FAILURE",ApplePaySession.STATUS_FAILURE);
...
function callApplePay(
  ...
  console.log("API URL",server + "/api/v1/payments/apple/" + paymentId + "/pay");
